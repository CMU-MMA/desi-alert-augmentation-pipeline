...
def test_run_from_a_stage_skips_the_earlier_ones(
    ...
    supplied = StageResult(stage=CROSSMATCH_STAGE, frame=gold_standard_alerts)
    results = pipeline.run_pipeline(pipeline_config, start=SLACK_STAGE, inputs={CROSSMATCH_STAGE: supplied})

    assert QUERY_STAGE not in results
    assert results[CROSSMATCH_STAGE] is supplied
